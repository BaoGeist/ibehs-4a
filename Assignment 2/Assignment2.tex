...
\item Question 3
  ...
  \item Question 4
    ...
    With these new transfer functions, we can relate the inputs $C'_{A0}$ and $Q'$ to the outputs $C'_A$ and $T'$ in the block diagram. Since we are required to relate the $C_{A0}(s)$ and $Q(s)$, we first subtract the equilibrium value and then we also add the equilibrium value back in the end to get the final transfer functions.

    \begin{figure}[H]
      \centering
      \includegraphics[width=0.6\textwidth]{Figures/handcalc/figure4-4e.png}
      \caption{Block Diagrams Relating $C_{A0}(s)$ and $Q(s)$ to $C_A(s)$ and $T(s)$}
      \label{fig:figure49}
    \end{figure}
