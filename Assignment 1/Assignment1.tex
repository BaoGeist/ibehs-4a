...
\item Question 3
  ...
  % ANSWER TO 3.2
  \item 
    The results of the Matlab code was plotted below in Figure 3. You can see that it plateaus to a value of 0.1. We know $V_s(t)$ was defined as a constant $5 V$ and R as $50 \Omega$. We know the steady state is $\frac{V_s(t)}{R}$ which in this case would give us $0.1 A$, so we can see that the plot shows our steady state expression holds.
...
\item Question 5
  ...
  \item Part C
  As you can see in Figure 7 below, plotting both the numerical solution (using ode45) and the analytical solution (the time domain solution we solved in 5.2), they are equivalent over the time shown.
