...
\end{figure}
  ...
  The simulation results align with the expected behavior of the system. The room temperature \( T(t) \) is maintained within the desired range of \( 17^\circ \text{C} \) to \( 23^\circ \text{C} \) as the furnace responds to changes in the ambient temperature \( T_a(t) \). When \( T_a(t) \) is lower, the furnace operates more frequently to offset increased heat loss, while at higher \( T_a(t) \), the furnace operates less often due to reduced heat loss. By summing the energy added to the room via the furnace over the time it is on over the 24-hours, we get find that 18492000 calories of energy were added to the room when the temperature fluctuates. Comparing it to the consistent minimum ambient temperature from Question 2.2 of 21276000 calories, we notice an decrease of 2784000 calories added to the room by the furnace due to the fluctuating ambient temperature. This is expected as the furnace is on less often when the ambient temperature is higher, and thus less energy is added to the room.
